...
library ValidateSPV {
    ...
    /// @param _tx      Raw bytes tx
    /// @return         txid
    function parseTransactionReturnTxid(bytes _tx) public pure returns (bytes32 _txid) {
        // These can probably be memory
        bytes memory componentsOfTx;
        bytes memory a;
        bytes memory b;

        // Extract prefix and validate, if invalid, bubble up error
        a = _tx.extractPrefix();
        if (!validatePrefix(a)) { return; }

        componentsOfTx = abi.encodePacked(a.slice(0, 4));

        // Extract number of inputs and inputs, if _nInputs is 0, bubble up error
        (a, b) = extractAllInputs(_tx);
        if (keccak256(a) == keccak256(hex'00')) { return; }
        componentsOfTx = abi.encodePacked(componentsOfTx, a, b);

        // Extract number of outputs and outputs, if _nOutputs is 0, bubble up error
        (a, b) = extractAllOutputs(_tx);
        if (keccak256(a) == keccak256(hex'00')) { return; }
        componentsOfTx = abi.encodePacked(componentsOfTx, a, b);

        // Extract locktime
        componentsOfTx = abi.encodePacked(componentsOfTx, _tx.extractLocktimeLE());

        // Calculate txid
        _txid = componentsOfTx.hash256();
    }

    /// @notice         Turns an address in bytes type format to an address type format
    /// @param _address Address in bytes
    /// @return         address
    function bytesToAddress(bytes _address) public pure returns (address) {
        require(_address.length == 20, "Data length != 20 bytes");
        uint160 m = 0;
        uint160 b = 0;
